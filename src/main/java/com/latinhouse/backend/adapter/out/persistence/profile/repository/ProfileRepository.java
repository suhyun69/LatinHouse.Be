...
public interface ProfileRepository extends JpaRepository<ProfileT, String> {
    @Query(value = """
        select * 
        from profiles
        where (:sex is null or sex = :sex)
            and (:isInstructor is null or is_instructor = :isInstructor)
        order by updated_at desc
    """, nativeQuery = true)
    List<ProfileT> findAll(@Param("sex") String sex, @Param("isInstructor") Boolean isInstructor);
